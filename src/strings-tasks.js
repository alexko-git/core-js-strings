...
function invertCase(str) {
  return str.replace(/[a-zA-Z]/g, (char) =>
    char.charCodeAt(0) >= 97 ? char.toUpperCase() : char.toLowerCase()
  );
}
...
function encodeToRot13(str) {
  function isUpperAlpha(letter) {
    return /[A-Z]/.test(letter);
  }
  function isLowerAlpha(letter) {
    return /[a-z]/.test(letter);
  }
  function encodeLetter(letter) {
    if (isLowerAlpha(letter)) {
      return String.fromCharCode(97 + ((letter.charCodeAt(0) - 97 + 13) % 26));
    }
    if (isUpperAlpha(letter)) {
      return String.fromCharCode(65 + ((letter.charCodeAt(0) - 65 + 13) % 26));
    }
    return letter;
  }
  ...
  return str.split('').map(encodeWord).join('');
}
